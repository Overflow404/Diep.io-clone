...
export class TwinGun implements ActionBehavior {

    private canvasContext: CanvasRenderingContext2D
    private world: World
    private bullets = new Array<Bullet>()
    private bulletDurationX = 60
    private bulletDurationY = 60
    static canFire = true
    ...
    fire(playerX: number, worldX: number, playerY: number, worldY: number, alpha: number) {
        if (TwinGun.canFire) {
            let bullet1 = new Bullet(playerX + worldX, playerY + worldY, 15, 15, playerX, playerY, alpha, true, 5)
            let bullet2 = new Bullet(playerX + worldX, playerY + worldY, 15, 15, playerX, playerY, (alpha - Math.PI), true, 5)
