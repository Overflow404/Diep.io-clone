...
export class StreamLiner implements ActionBehavior {
    ...
    fire(playerX: number, worldX: number, playerY: number, worldY: number, alpha: number) {
        if (StreamLiner.canFire) {
            let bullet = new Bullet(playerX + worldX, playerY + worldY, 15, 15, playerX, playerY, alpha, true, 8)
    ...
    detectCollision(obstacles: Array<Obstacle>, i: number, x: number, y: number) {
        for (var j = 0; j < obstacles.length; j++) {
            ...
            let cornerDistanceSq = (circleDistanceX - rectW / 2) ^ 2 +
